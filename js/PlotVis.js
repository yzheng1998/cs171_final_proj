...
class PlotVis {
    constructor(parentElement, movieData, streamingData) {
    ...
    wrangleData() {
        ...
        for (const platform in vis.movieData) {

            platformCounts[platform] = vis.movieData[platform].size
        }
        ...
        document.getElementById("disney-count").innerHTML = platformCounts['Disney+'];
        document.getElementById("prime-video-count").innerHTML = platformCounts['Prime Video'];


        // for (const platform in vis.movieData) {
        //     console.log(`${platform}: ${vis.movieData[platform]}`);


            vis.movieData[plotPlatform].forEach(id => {
                let index = vis.streamingData.findIndex(x => x.Id === id);
                // vis.displayData.push({
                //     netflix: vis.streamingData[index].Netflix,
                //     hulu: vis.streamingData[index].Hulu,
                //     disney: vis.streamingData[index]['Disney+'],
                //     amazon: vis.streamingData[index]['Prime Video'],
                //     imdb: vis.streamingData[index].IMDb,
                //     rotten: vis.streamignData[index].
                // })


                if (!vis.displayData.includes(vis.streamingData[index])
                    && vis.streamingData[index]['Rotten Tomatoes'] !== null) {
                    vis.displayData.push(vis.streamingData[index]);
                }

                // console.log(vis.streamingData[index]['Rotten Tomatoes']);
            });

        // }

        // vis.displayData.forEach(item => {
        //     console.log(typeof(item['Netflix']))
        // })

        // vis.displayData.filter(item => {
        //     return item["Netflix"] === 1;
        // })
        //
        ...
        // console.log(typeof(vis.displayData[0]['Rotten Tomatoes']));
    ...
    updateVis() {
        ...
        vis.x
            .domain(d3.extent(vis.displayData, d => d['Rotten Tomatoes']))
            // .domain(d3.extent(vis.displayData, d => d['Rotten Tomatoes'])).nice()

        vis.y
            .domain(d3.extent(vis.displayData, d => d['IMDb']))
            // .domain([0, 10])
        ...
        circle.enter().append("circle")
            ...
            .on("mouseover", function(event, d) {
                ...
                let html  = "<span style='font-weight: bold;'>" + d.Title + "</span><br/>" +
                    "<span>" + "Genres: " + d.Genres.replace(/,(?=[^\s])/g, ", ") + "</span><br/>" +
                    "<span>" + "IMDb score: " + d.IMDb + "</span><br/>" +
                    "<span>" + "Rotten Tomatoes score: " + d['Rotten Tomatoes'] + "</span><br/>";
